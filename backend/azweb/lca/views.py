...
def user_register(request):
    ...
    template = 'lca/registration.html'

    if request.method == 'POST':
        # create a form instance and populate it with data from the request:
        form = Lca_Form(request.POST)
        ...
        if form.is_valid():
            ...
            if  not (Azeo_id_user.objects.filter(azeo_id=form.cleaned_data['member1']).exists()) :
            ...
            elif Lcauser.objects.filter(member1=form.cleaned_data['member1']).exists() :
                return render(request, template, {
                    'form': form,
                ...
                subject = "Successfully registered for LCA Competition "
                ...
                html_message = render_to_string("lca/mail.html",{'name':name1})
                ...
                return render(request, "lca/confirmation.html",)
